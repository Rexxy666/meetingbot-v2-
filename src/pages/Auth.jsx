...
export default function Auth({ auth }) {
  ...
  // Google 為主：Email 登入預設收起。未設定 Firebase 時（本機 dev）自動展開 Email。
  const [showEmail, setShowEmail] = useState(!isFirebaseConfigured);
    ...
    <div className="min-h-screen flex items-center justify-center px-6 py-12 bg-gradient-to-b from-mint-50/80 to-white">
      <div className="w-full max-w-md">
        ...
        <div className="bg-white border border-navy-800/8 rounded-3xl shadow-card p-6">
          ...
          {isFirebaseConfigured && (
            <div className="mb-1">
              <button
                ...
                disabled={busy}
                onClick={googleLogin}
                className="w-full inline-flex items-center justify-center gap-3 font-bold py-3.5 rounded-2xl border border-navy-800/10 bg-white text-navy-800 shadow-card hover:shadow-card-hover hover:border-navy-800/15 disabled:opacity-60 transition-all active:scale-[0.99]"
              >
                <GoogleGlyph />
                {busy ? "處理中…" : "使用 Google 帳號繼續"}
              </button>
              <p className="mt-2.5 text-center text-[11px] text-navy-400">
                免記密碼、更安全，一鍵登入或註冊
              </p>
            </div>
          )}
          ...
          {error && (
            <p className="mt-4 text-sm text-coral-500 bg-coral-50 border border-coral-100 rounded-xl px-3 py-2">{error}</p>
          )}

          {/* ── 次要方式：Email 登入（預設收起）── */}
          {isFirebaseConfigured && !showEmail ? (
            <div className="mt-5">
              ...
              <button
                ...
                className="w-full font-semibold py-3 rounded-2xl border border-navy-800/10 bg-navy-800/[0.02] text-navy-600 hover:bg-navy-800/[0.05] transition-colors"
              >
                改用 Email 登入 / 註冊
              </button>
            </div>
          ) : (
            <div className={isFirebaseConfigured ? "mt-5 pt-5 border-t border-navy-800/8" : ""}>
              <div className="flex gap-1 p-1 rounded-2xl bg-navy-800/[0.03] mb-5">
                <button
                  type="button"
                  onClick={() => {
                    setMode("login");
              ...
              {isFirebaseConfigured && (
                <button
                  ...
                  className="mt-4 w-full text-center text-[12px] font-semibold text-navy-400 hover:text-navy-600 transition-colors"
                >
                  ← 返回，改用 Google 登入
                </button>
              )}
